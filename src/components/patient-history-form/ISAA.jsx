...
const ISAA = () => {
  ...
  const { dataCollectionMode } = location.state || { dataCollectionMode: [] };  // Retrieving dataCollectionMode from the state
  ...
  const handleNext = (e) => {
    ...
    try {
      ...
      // Check if 'CARS' is in the dataCollectionMode array
      if (dataCollectionMode.includes('CARS')) {
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.sidebar}>
        ...
        <div style={styles.sidebarContent}>
          ...
          <p style={styles.sidebarDetails}>
            Directions:
            Below are given 40 statements which are divided under six domains, please select the appropriate rating for each item of the scale by observing the child and by
      ...
      <div style={styles.formContainer}>
        ...
        {[
  { title: "SOCIAL RELATIONSHIP AND RECIPROCITY", start: 1, end: 9, questions: [
  ...
  { title: "EMOTIONAL RESPONSIVENESS", start: 10, end: 14, questions: [
    "Shows inappropriate emotional response", "Shows exaggerated emotions", "Engages in self-stimulating emotions", 
    "Lacks fear of danger", "Excited or agitated for no apparent reason"
  ]},
  { title: "SPEECH-LANGUAGE AND COMMUNICATION", start: 15, end: 23, questions: [
    "Acquired speech and lost it", "Has difficulty in using non-verbal language or gestures to communicate", 
    "Engages in stereotyped and repetitive use of language", "Engages in echolalic speech", 
    "Produces infantile squeals/ unusual noises", "Unable to initiate or sustain conversation with others", 
    "Uses jargon or meaningless words", "Uses pronoun reversals", "Unable to grasp pragmatics of communication (real meaning)"
  ]},
  { title: "BEHAVIOUR PATTERNS", start: 24, end: 30, questions: [
    "Engages in stereotyped and repetitive motor mannerisms", "Shows attachment to inanimate objects", 
    "Shows hyperactivity/ restlessness", "Exhibits aggressive behavior", "Throws temper tantrums", 
    "Engages in self-injurious behavior", "Insists on sameness"
  ]},
  { title: "SENSORY ASPECTS", start: 31, end: 36, questions: [
    "Unusually sensitive to sensory stimuli", "Stares into space for long periods of time", 
    "Has difficulty in tracking objects", "Has unusual vision", "Insensitive to pain", 
    "Responds to objects/people unusually by smelling, touching or tasting"
  ]},
  { title: "COGNITIVE COMPONENT", start: 37, end: 40, questions: [
    ...
    <table style={styles.table}>
      ...
      <tbody>
        {section.questions.map((question, index) => (
          <tr key={index}>
            <td style={styles.tableCell}>{`${section.start + index}. ${question}`}</td>
            {[1, 2, 3, 4, 5].map((value) => (
